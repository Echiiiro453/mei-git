...
# ... (As funções auxiliares e de interface continuam as mesmas) ...
export LANG=C.UTF-8
LOG_FILE="/tmp/mei-git-setup.log"
>"$LOG_FILE"
install_dialog_if_missing() { #... (código omitido por brevidade)
...
run_install_arch_family() {
    ...
    (
        ...
        if command -v yay &> /dev/null; then
            ...
            # Se o yay não for encontrado, o status será de sucesso parcial
            # e a mensagem final irá instruir o usuário.
    ...
    # Verifica se o yay existe para a mensagem final
    if ! command -v yay &> /dev/null; then
        YAY_MISSING=true
    fi
}
...
main() {
    ...
    case "$OS" in
        "ubuntu" | "debian" | "linuxmint" | "deepin" | "pop" | "mx")
            # A lógica para Debian-based continua a mesma...
            # ... (código omitido por brevidade)
            ;;
        "fedora" | "rhel" | "centos")
            # A lógica para Fedora-based continua a mesma...
            # ... (código omitido por brevidade)
            ;;
    ...
    # Se o yay estava faltando, dá instruções extras para o usuário
